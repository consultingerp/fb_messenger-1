...
class WkCOD(models.Model):
    ...
    @api.onchange('min_order_amount', 'max_order_amount')
    def onchangeMinMaxAmount(self):
        for codObj in self:
            ...
            codObj.policy_content = "Order Order Amount Must Be in Between {} to {}".format(min_order_amount, max_order_amount)
...
class website(models.Model):
    ...
    def expected_cod_date(self, product):
        ...
        res = (date.today() + relativedelta(days=+int(product.sale_delay))).strftime("%d.%m.%Y")
        return res
